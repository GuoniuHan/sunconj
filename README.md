...
This generates the output file `output_frac_minus_1000.txt.gz`
,
from which the prime sets are extracted and provided in
`data_frac_minus_1000.txt`.

### Proof of Theorem 5.2 for s = +1

To reproduce the result of Theorem 5.2 for s = +1, run the following command
using the program `sunconj_prime.py`:

```
python sunconj_prime.py 100000 +1 true true > output_prime_plus_100000.txt
```

This generates the output file `output_prime_plus_100000.txt`
,
from which the prime sets are extracted and provided in
`data_prime_plus_100000.txt`.
...
This generates the output file `output_prime_minus_100000.txt`
,
from which the prime sets are extracted and provided in
`data_prime_minus_100000.txt`.

### Proof of Theorem 5.3 for s = +1

To reproduce the result of Theorem 5.3 for s = +1, run the following command
using the program `sunconj_power.py`:

```
python sunconj_power.py 40 +1 true true > output_power_plus_40.txt
```

This generates the output file `output_power_plus_40.txt`,
from which the prime sets are extracted and provided in
`data_power_plus_40.txt`.
...
This generates the output file `output_power_minus_40.txt`,
from which the prime sets are extracted and provided in
